...
void AppStateManager::start(AppState* state)
  {
    ...
    double currentTime = OgreFramework::getSingletonPtr()->getTimerPtr()->getMilliseconds() / 1000.0f;
    double accumulator = 0.0;

    while ( !m_bShutdown )
      {
	if ( OgreFramework::getSingletonPtr()->getRenderWindowPtr()->isClosed() )
	  m_bShutdown = true;

	Ogre::WindowEventUtilities::messagePump();

	if ( OgreFramework::getSingletonPtr()->getRenderWindowPtr()->isActive() )
	  {
	    double newTime = OgreFramework::getSingletonPtr()->getTimerPtr()->getMilliseconds() / 1000.0f;
	    ...
	    while ( accumulator >= dt )
	      {
		OgreFramework::getSingletonPtr()->getKeyboardPtr()->capture();
		OgreFramework::getSingletonPtr()->getMousePtr()->capture();
    ...
    OgreFramework::getSingletonPtr()->getLogMgrPtr()->logMessage("Main loop quit");
  }
...
void AppStateManager::init(AppState* state)
  {
    OgreFramework::getSingletonPtr()->getKeyboardPtr()->setEventCallback ( state );
    OgreFramework::getSingletonPtr()->getMousePtr()->setEventCallback ( state );
    OgreFramework::getSingletonPtr()->getSDKTrayMgrPtr()->setListener ( state );

    OgreFramework::getSingletonPtr()->getRenderWindowPtr()->resetStatistics();
  }
