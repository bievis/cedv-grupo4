...
void Records::add ( int hostages, int seconds )
  {
    ...
    sprintf ( cad, "%d|%d|%s|%s", hostages, seconds, fecha.c_str(), hora.c_str() );
    ...
    if ( _records.size() == 0 )
    ...
    else
      {
        for ( it = _records.begin(); it != _records.end(); ++it )
          {
            if ( compare ( hostages, seconds, *it ) > 0 ) //newValue > *it
              {
                break;
              }
          }
...
int Records::compare ( int hostages, int seconds, string value2 )
  {
    ...
    sscanf ( value2.c_str(), "%d|%d|%s", &hostages2, &seconds2, resto );

    if ( ( hostages > hostages2 ) ||
        ( hostages == hostages2 && seconds < seconds2 ) )
      {
        ret = 1;
      }
